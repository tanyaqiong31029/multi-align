...
```bash
node agent-skill/scripts/pipeline_test.js          # 默认测试仓库自身（43 项断言）
...
1. **回归测试**：43 项断言，覆盖多语分句（缩写/小数/引号/有序列表标号）、对齐珠型、翻译单元合并、导出器 ZIP/XML 完整性与 openpyxl 打开校验、DOCX 导入、GBK 编码识别、SRT/VTT 字幕全链路、导出编排层与统计模块默认静默；
