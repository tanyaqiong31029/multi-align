...
- **隐私安全**：全部计算在浏览器本地完成，文本不上传任何服务器；工程可存 JSON、浏览器自动保存防丢失
...
```bash
node agent-skill/scripts/pipeline_test.js          # 默认测试仓库自身（34 项断言）
...
1. **回归测试**：34 项断言，覆盖多语分句（缩写/小数/引号）、对齐珠型、翻译单元合并、导出器 ZIP/XML 完整性与 openpyxl 打开校验、DOCX 导入、GBK 编码识别与 SRT/VTT 字幕全链路；
